...
public interface WebResourceRoot extends Lifecycle {
    ...
    /**
     * 资源文件的加载方式，有四种：
     *  1.通过pre方式，在Context.xlm中<PreResource></PreResource>节点下
     *  2.通过main方式：在WEB-INF/lib classes文件夹下
     *  3.通过jar包方式：在<jarResource></jarResource>
     *  4.通过post方式:在<postResource></postResource>节点下，通过post方式配置
     * @param path
     * @return
     */
